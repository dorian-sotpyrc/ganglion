...
def extract_session_messages(session_jsonl: Path, limit: int = 12) -> list[str]:
    ...
    for line in session_jsonl.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if isinstance(content, str):
            ...
            texts = [str(chunk.get("text", "")).strip() for chunk in content if isinstance(chunk, dict) and chunk.get("type") == "text"]
...
def main() -> None:
    ...
    if sessions_index:
        newest = max(sessions_index.values(), key=lambda item: item.get("updatedAt", 0))
    ...
    output_path = out_dir / "memory_bundle.json"
    output_path.write_text(json.dumps(bundle, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"ok": True, "output": str(output_path), "critical": len(critical), "episodic": len(episodic)}, indent=2))
